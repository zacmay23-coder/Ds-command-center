...
test("administrator maintenance endpoints remain in the authenticated API router", async () => {
  const source = await readFile(new URL("../src/http/apiRouter.js", import.meta.url), "utf8");
  const router = source.slice(source.indexOf("export async function handleApi"), source.indexOf("function normalizeApiPath"));
